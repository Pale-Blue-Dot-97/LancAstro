...
def make_bins(a, bin_min=None, bin_max=None, bin_wid=1, n=10):
    """Bins the supplied data according to parameters supplied

    Args:
        a: Data to be binned
        bin_min: 'left' edge of minimum bin
        bin_max: 'right' edge of maximum bin
        bin_wid: Width of bins
        n: Number of bins

    Returns:
        counts: Number of counts in each bin
        bins: Mid-point of each bin
        bin_min: 'left' edge of minimum bin
        bin_max: 'right' edge of maximum bin
        bin_wid: Width of bins
...
def output_bins_to_plot(a, bin_min=None, bin_max=None, bin_wid=1, n=10):
    """Bins supplied data then transforms into x-y points ready to plot a histogram from
    ...
    Returns:
        x: x-axis data of histogram
        y: y-axis data of histogram
        bin_min: 'left' edge of minimum bin
        bin_max: 'right' edge of maximum bin

    """

    # Uses make_bins to bin data
    counts, bins, bin_min, bin_max, bin_wid = make_bins(a, bin_min, bin_min, bin_wid, n)

    # Constructs the extra points either side of the mid-point of the bin to plot
